...
class SubplotAnimation(animation.TimedAnimation):
    def __init__(self,t1_val,t2_val,mid_val,log_path,
        bin_size=100,target_pause=4.3,timeout_pause=11.3):
        ##set up our figure
        fig = plt.figure()
        ...
        ##add labels, etc
        ax1.set_ylabel('Value, A.U.',fontsize=20)
        ax1.yaxis.set_label_position("right")
        ax1.set_xlabel('Bins',fontsize=20)
        ax1.set_ylim(t2_val-0.15,t1_val+0.15)
        ax1.set_xlim(-self.show_n, 2) ##this should give us 2 secs of data visible
        ax1.set_title("Cursor value",fontsize=20,weight='bold')
        ...
        ax2.set_ylabel('Sum of E1 spikes',fontsize=20)
        ax2.set_xlabel('Bins',fontsize=20)
        ax2.set_title("Ensemble 1 value",fontsize=20,weight='bold')
        self.line2 = Line2D([], [], color='green',linewidth=2)
        ax2.add_line(self.line2)
        ax2.set_xlim(-self.show_n, 2)
        ax2.set_ylim(0, max(np.nanmax(self.e1),np.nanmax(self.e2)))
        ...
        ax3.set_xlabel('Bins',fontsize=20)
        ax3.set_ylabel('Sum of E2 spikes',fontsize=20)
        ax3.set_title("Ensemble 2 value",fontsize=20,weight='bold')
        self.line3 = Line2D([], [], color='blue',linewidth=2)
        ax3.add_line(self.line3)
        ax3.set_xlim(-self.show_n, 2)
        ax3.set_ylim(0, max(np.nanmax(self.e2),np.nanmax(self.e1)))
    ...
    def parse_log(self):
        ...
        target_pause_bins = int(self.target_pause*1000)/self.bin_size
        timeout_pause_bins = int(self.timeout_pause*1000)/self.bin_size
...
ani = SubplotAnimation(3.09798,-0.46962,1.3,r"C:\Users\Ryan\Desktop\BMI_D08.txt")
#ani.save(r"C:\Users\Ryan\Desktop\BMI_ani.mp4")
plt.show()
